...
/*
El modulo regs16 es un modulo que simula un registro que puede
almacenar datos de hasta 16 bits de tamaño, en este caso datos del tipo
sc_unit<16>
...
class regs32 : public sc_module{
	public:
		...
		sc_in < sc_int<32> > data_in; //la data que entra al modulo, del TIPO SC_UNIT<16>, que es nos permite almacenar numeros de hasta 8 bits
		sc_out < sc_int<32> > data_out; //la data que sale del modulo, del TIPO SC_UNIT<16>, que es nos permite almacenar numeros de hasta 16 bits

		sc_int<32> data; //la que ESTA en el modulo actualmente, es decir, la que saldra por data_out
	...
	private:
void reading_data(){
		//if (enable_in.read() == true)		//SOLO SI ENABLE ES 1/True, escribo lo que hay en "data" lo que entra por data_in	                                //de lo contrario, "data" mantendra su valor actual
		//{
		  data = data_in.read();
		//}
	}
